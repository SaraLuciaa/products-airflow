...
def main():
    ...
    revision = eda.get("revision_inicial", {})  # por si quieres usarlo después
    stats = eda.get("estadisticas", {})

    global_info = stats  # para mantener el nombre mental
    ...
    # Días pico de compra (día de la semana)
    with c1:
    ...
    # Categorías más “rentables” (por volumen / porcentaje)
    with c2:
    ...
    # ----------------------------------------------------------------------
    # Serie de tiempo: ventas por día o semana
    # ----------------------------------------------------------------------
    ...
    with c_ts:
        ...
        if ts_gran == "Diaria":
            ...
            if semanal:
                ...
                # Construimos una etiqueta año-semana
                df_sem["periodo"] = (
    ...
    # ----------------------------------------------------------------------
    # Boxplot: totales por cliente o categoría (Top 10)
    # ----------------------------------------------------------------------
    with c_box:
    ...
    # Usamos métricas de Top 10 clientes como ejemplo:
    top_clients = dist.get("top_10_clientes_mas_compras", {}).get("top_10", [])
    if top_clients and len(top_clients) > 1:
        df_cli = pd.DataFrame(top_clients)

        # Nos quedamos con columnas numéricas relevantes
        numeric_cols = [
            c
            for c in df_cli.columns
            if pd.api.types.is_numeric_dtype(df_cli[c])
        ]
    ...
    # (Opcional) podrías agregar más secciones usando revision_inicial y
    # estadísticos numéricos si lo necesitas para el informe.
